...
class RotacaoSist(Scene):
    def construct(self):
        t = Text(
            "Rotação dos Sistemas de Coordenadas",
            t2c = {'[10:31]': GREEN_A}
        )
        self.play(Write(t), run_time=5)
        self.wait(2)
...
class SO2(Scene):
    def construct(self):
        t = Text("Demonstração", color=PURPLE)
        self.play(Write(t), runt_time=4)
        self.wait(2)
        self.play(FadeOut(t))
        self.wait()

class defOpBinaria(Scene):
    def construct(self):
        G = MathTex(r"G = \{a, b, c, d, ... \}")
        G1 = MathTex(r"*: \ G \times G \rightarrow G")
        G2 = MathTex(r" ( a, b ) \mapsto a * b")
        VGroup(G, G1, G2).arrange(DOWN).scale(1.5)
        self.play(Write(G), run_time = 5)
        self.play(Write(G1), run_time = 15)
        self.play(Write(G2), run_time = 20)
        self.wait(10)
        self.play(FadeOut(G), FadeOut(G1), FadeOut(G2))
        self.wait(2)

class defGrupo(Scene):
    def construct(self):
        G = MathTex(r"G \neq \emptyset")
        G1 = MathTex(r"*: \ G \times G \rightarrow G")
        G2 = VGroup(G, G1).arrange(DOWN).scale(3)
        G3 = MathTex(r"(G, *)").scale(2.5)
        P1 = MathTex(r"\forall \ a, b \in G \Rightarrow a * b \in G").scale(1.5)
        P2 = MathTex(r"\forall \ a,b \in G \Rightarrow (a * b) * c = a * (b * c)").next_to(P1, DOWN).scale(1.5)
        P3 = MathTex(r"\exists \ e \in G \ | \ \forall \ a\in G \Rightarrow e * a = a").next_to(P2, DOWN).scale(1.5)
        P4 = MathTex(r"\forall \ a\in G \Rightarrow \exists \ a^{-1} \in G \ | \ a^{-1} * a = e").next_to(P3, DOWN).scale(1.5)
        self.play(Write(G), run_time = 6)
        self.play(Write(G1), run_time = 10)
        self.play(ReplacementTransform(G2, G3), run_time = 2)
        self.wait(2)
        self.play(G3.animate.shift(2.5*UP))
        #self.play(FadeOut(G), FadeOut(G1), FadeOut(G2), FadeOut(G3))
        self.play(Write(P1), run_time = 10)
        self.wait(10)
        self.play(Write(P2), run_time = 20)
        self.wait(12)
        self.play(Write(P3), run_time = 20)
        self.wait(10)
        self.play(Write(P4), run_time = 20)
        self.wait(10)
        self.play(FadeOut(G3), FadeOut(P1), FadeOut(P2), FadeOut(P3), FadeOut(P4))
        self.wait(4)
